        ...
        <FormItem label="商品分类">
          <div>
            ...
            <Poptip v-model="showPoptip" placement="left-end" width="280">
              ...
              <Button icon="ios-add" type="dashed" size="small">添加分类</Button>
            </Poptip>
          </div>
        </FormItem>
...
export default {
  ...
  computed: {
    ...
    _categoryList () {
      const oneList = this.categoryList.find(item => item.id === this.categoryParent)
      if (oneList && oneList.children) { return oneList.children }
      return []
    }
  },
  ...
  methods: {
    ...
    getJoinCategory (idsArr) {
      ...
      const done = (id, _tempObj) => {
        ...
        if (typeof _tempObj !== 'undefined') {
          ...
          current = this.categoryListEqual.find(v => v.id === id)
        }

        const parent = this.categoryListEqual.find(v => v.id === current.pid)
    ...
    getHistoryCategorys () {
      ...
      if (historyCategorys) { return JSON.parse(historyCategorys) }
      return []
    },
