        ...
        <Col style="min-width: 320px" :lg="{ span: 8, offset: 1 }" :sm="{ span: 12, offset: 0 }">
          <Form ref="formData" :model="formData" :rules="formValidate" :label-width="90">
            ...
            <FormItem label="主图" prop="cover">
              <div class="upload-list upload-list-1" v-for="(item, index) in uploadList1" :key="index">
            ...
            <FormItem label="轮播图" prop="carousels">
              <div class="upload-list upload-list-2" v-for="(item, index) in uploadList2" :key="index">
                <template v-if="item.status === 'finished'">
                  <img :src="item.url">
                  <div class="upload-list-cover">
                    <Icon type="ios-eye-outline" @click.native="handleViewImg(item.url)"></Icon>
