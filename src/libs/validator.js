...
export function checkMobile (mobile) {
  if (/^((\d{3}-\d{8}|\d{4}-\d{7,8})|(1[3|5|7|8][0-9]{9}))$/.test(mobile)) { return true }
  return false
}
...
export function checkTaxNo (taxNo) {
  if (/^[A-Z0-9]{15}$|^[A-Z0-9]{17}$|^[A-Z0-9]{18}$|^[A-Z0-9]{20}$/.test(taxNo)) { return true }
  return false
}
...
export function checkUserName (username) {
  if (/^[a-zA-Z0-9_-]{3,16}$/.test(username)) { return true }
  return false
}
...
export function checkIdCard (obj) {
  if (/(^\d{15}$)|(^\d{18}$)|(^\d{17}(\d|X|x)$)/.test(obj)) { return true }
  return false
}
...
export function checkGATIdCard (obj) {
  ...
  if (reg1.test(obj) || reg2.test(obj) || reg3.test(obj) || reg4.test(obj)) { return true }
  return false
}
